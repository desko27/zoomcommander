...
void ZoomNodeMeetingParticipantsCtrlWrap::SetMeetingUserNameChangeCB(const v8::FunctionCallbackInfo<v8::Value> &args)
{
	v8::Isolate* isolate = args.GetIsolate();
	if (args.Length() < 1) {
		isolate->ThrowException(v8::Exception::TypeError(
	...
	if (args[0]->IsNull())
	{
		ZoomNodeSinkHelper::GetInst().onUserNameChange.Empty();
		return;
	}
	if (!args[0]->IsFunction())
	{
	...
	v8::Local<v8::Function> cbfunc = v8::Local<v8::Function>::Cast(args[0]);
	v8::Persistent<v8::Function, v8::CopyablePersistentTraits<v8::Function> > cb(isolate, cbfunc);
	ZoomNodeSinkHelper::GetInst().onUserNameChange = cb;
...
void ZoomNodeMeetingParticipantsCtrlWrap::GetUserInfoByUserID(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	node->Set(context, v8::String::NewFromUtf8(isolate, "audioStatus", v8::NewStringType::kInternalized).ToLocalChecked(), v8::Integer::New(isolate, (int32_t)zn_userInfomation.audioStatus));
