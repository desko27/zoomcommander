...
void ZoomNodePremeetingWrap::SetonGetInviteEmailContentCB(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	ZNSDKError err = ZNSDKERR_SUCCESS;
	do
	{
		...
		if (args[0]->IsNull())		{			ZoomNodeSinkHelper::GetInst().onGetInviteEmailContent.Empty();			err = ZNSDKERR_INVALID_PARAMETER;
			break;;		}
		if (!args[0]->IsFunction())
		{
			err = ZNSDKERR_INVALID_PARAMETER;
		...
		v8::Local<v8::Function> cbfunc = v8::Local<v8::Function>::Cast(args[0]);
		v8::Persistent<v8::Function, v8::CopyablePersistentTraits<v8::Function> > cb(isolate, cbfunc);
		ZoomNodeSinkHelper::GetInst().onGetInviteEmailContent = cb;
